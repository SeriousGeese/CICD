// PreToolUse(Bash) hook: block a command that contains a doubled backslash (`\\`).
//
...
try {
  ...
  // Registered on the Bash matcher; PowerShell preserves `\\` and must never be blocked.
  ...
  process.stderr.write(
    `Blocked: this command contains \`\\\\\` (${hits} occurrence${hits === 1 ? '' : 's'}). The Bash tool's ` +
      ...
      'receives `\\\\` intact, and the Write, Edit and PowerShell tools all preserve it — the collapse is the ' +
      'Bash tool channel alone. Put the content in a file with the Write tool and run or consume the FILE ' +
      '(`bash <path>`, `node <path>`, `sed -f <path>`), use the Grep tool for regex searches, or write a ' +
      'single `\\` when a single one is what you mean. If you have deliberately pre-doubled for the collapse, ' +
      `set AGENT_HOOKS_ALLOW_DOUBLE_BACKSLASH=1. ${NOTHING_RAN_NOTICE}${consumerHint(import.meta.url)}`,
  );
  process.exit(2);
} catch {
  process.exit(0);
